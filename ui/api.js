...
export async function getOrCreateProfile() {
  ...
  if (existingId) {
    try {
      return await json(await fetch(`/api/player/${existingId}`));
    } catch (err) {
      // Falls through to create a fresh profile if the saved id is stale/missing server-side.
    }
  }
  const profile = await json(await fetch('/api/player', {
    method: 'POST',
...
export async function renamePlayer(id, name) {
  return json(await fetch(`/api/player/${id}/rename`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function selectCharacter(id, characterId) {
  return json(await fetch(`/api/player/${id}/select-character`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function unlockCharacter(id, characterId) {
  const res = await fetch(`/api/player/${id}/unlock-character`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ characterId }),
  });
...
export async function exchangeGems(id, gemCount) {
  const res = await fetch(`/api/player/${id}/exchange-gems`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function selectWorld(id, worldId) {
  return json(await fetch(`/api/player/${id}/select-world`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function saveRun(id, payload) {
  return json(await fetch(`/api/player/${id}/save-run`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function getQuests(id) {
  return json(await fetch(`/api/player/${id}/quests`));
}

export async function getLeaderboard(limit = 10, range = 'all') {
  return json(await fetch(`/api/leaderboard?limit=${limit}&range=${range}`));
}
